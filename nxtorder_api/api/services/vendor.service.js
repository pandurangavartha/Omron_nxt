...
const VendorService = require('../models/vendors.model');
...
exports.registerUser = function (userObj) {
    return VendorService.findOne({
        $or: [
            ...
            .then(function (user) {
//                co
                console.log(user, "usersuseruser")
                if (user) {
                    var err = new Error(Msg.USER_EXISTS);
                    err.status = 409;
                    return Promise.reject(err);
                }
                return VendorService.create(userObj)
                        .then(function (user) {
...
exports.updateProfile = function (user, userId, vendorObj) {
    vendorObj.user_id = userId
    ...
    return VendorService.findOne({
        ...
        return VendorService.update({_id: userId}, {$set: user}).then(function (data) {
            console.log("innnnn venodrs ooooooooooooo")
            return msg
        }).catch(function (err) {
            console.error(err);
            return Promise.reject(err);
