...
//        qrCodeImage:null,
        gender: null,
        languagesSpoken: [],
        currentPosition: [req.body.longitude, req.body.latitude]
    };    
    console.log(profilePercentage, 'profilePercentageprofilePercentage')

    
    
    if (userObj) {
        profilePercentage = 40;
        userObj.basicPercentage = 0;    
    }
    if (req.body.addresses) {
        profilePercentage = profilePercentage + 30;
        userObj.addressPercentage = 0;
    }
    userObj.addresses = req.body.addresses
    if (userObj.interests) {
        userObj.interests = JSON.parse(userObj.interests)
        profilePercentage = profilePercentage + 30;
        userObj.interestsPercentage = 0;
//        console.log(typeof userObj.interests, "userObjuserObj")
    ...
    UserService.registerUser(userObj).then(function (user) {
...
//    Request.get("http://www.metamorphsystems.com/index.php/api/bulk-sms?username=sciens &password=Sciens@123&from=NXTORD&to=" + req.body.username + "&sender=NXTORD&message=" + OTP, (error, response, body) => {
    Request.get("http://www.metamorphsystems.com/index.php/api/bulk-sms?username=sciens &password=Sciens@123&from=NXTORD&to=" + req.body.username + "&sender=NXTORD&message=" + OTP, (error, response, body) => {
        if (error)
            ...
            throw error;
//        
        UserService.updateProfileWithOtp(userObj, req.body.username)
                .then(function (user) {
...
exports.otpVerification = function (req, res, next) {
    if (!req.body.mobile) {
        return res.status(400).send({result: {error: Msg.BAD_REQUEST}})
    }
    if (!req.body.otp) {
    ...
    UserService.otpVerification(req.body.mobile, req.body.otp)
            .then(function (user) {
                return res.status(200).send({result: Msg.OTP_VERIFICATION});
//            return res.send({ result: user.responseJSON() });
            }).catch(function (err) {
        return res.status(400).send({result: {error: err.message}})
    });
...
//        
//    }
    UserService.updateProfile(userObj, req.params.user_id)
            .then(function (user) {
                return res.send({result: user.responseJSON()});
            }).catch(function (err) {
        return res.status(400).send({result: {error: err.message}})
...
exports.view = function (req, res, next) {
    ...
    UserService.getProfile(req.params.user_id).then(function (data) {
        if (data) {
...
exports.logout = function (req, res, next) {
    UserService.logout(req.params.user_id, req.headers[Msg.HEADER_OS_TYPE].toLowerCase(), req.body.deviceToken)
            .then(function (user) {
...
exports.activate = function (req, res, next) {
    UserService.profileActivate(req.params.user_id, req.query).then(function (user) {
        if (user) {
            return res.send({result: user.responseJSON()})
        } else {
            return res.status(400).send({result: {error: Msg.USER_NOT_EXISTS}})
...
exports.resetPassword = function (req, res, next) {
    UserService.setPassword(req.body, req.headers[Msg.HEADER_OS_TYPE].toLowerCase()).then(function (user) {
        if (user) {
            return res.send({result: user.responseJSON()})
        } else {
            return res.status(400).send({result: {error: Msg.USER_NOT_EXISTS}})
...
exports.all = function (req, res, next) {
    UserService.all(req.query).then(function (users) {
